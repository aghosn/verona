...
namespace verona::rt
{
  ...
  private:
    ...
    friend class ThreadSyncSystematic<WorkerThread>;
    Systematic::Local* local_systematic{nullptr};
#else
    friend class ThreadSync<WorkerThread>;
    LocalSync local_sync{};
#endif

    C* core = nullptr;
    Alloc* alloc = nullptr;
    std::atomic<C*> next = nullptr;
    C* victim = nullptr;
    ...
    WorkerThread() /*: token_cown{T::create_token_cown()}, q{token_cown}*/
    {
  ...
  inline void stop()
    ...
    static void run(WorkerThread* t, void (*startup)(Args...), Args... args)
    {
    ...
    void run_inner(void (*startup)(Args...), Args... args)
    {
      ...
      // for a different WorkerThread later.
    ...
    bool prerun(T* cown)
    {
      // See if this is a WorkerThread enqueued as a cown LD marker.
      // It may not be this one.
      if (has_thread_bit(cown))
      {
        auto unmasked = clear_thread_bit(cown);
        WorkerThread* sched = unmasked->owning_thread();
