...
std::map<std::string, TokenKind> builtins = {
    ...
    {std::string("("), LParen}, {std::string(")"), RParen},
    {std::string("="), Equals}, {std::string(";"), SemiColon},
};
...
bool Lexer::isEmpty() { return lines.size() == 0; }

bool Lexer::hasNext() { return (!this->isEmpty() && la != -1); }
